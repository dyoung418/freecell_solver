...
class SimpleProblemSolvingAgentProgram(object):
...
def breadth_first_tree_search(problem):
    ...
    return tree_search(problem, utils.FIFOQueue())

def depth_first_tree_search(problem):
    "Search the deepest nodes in the search tree first."
    return tree_search(problem, utils.Stack())
...
australia = UndirectedGraph(utils.Dict(
    T=utils.Dict(),
    SA=utils.Dict(WA=1, NT=1, Q=1, NSW=1, V=1),
    NT=utils.Dict(WA=1, Q=1),
    NSW=utils.Dict(Q=1, V=1)))
australia.locations = utils.Dict(WA=(120, 24), NT=(135, 20), SA=(135, 30),
